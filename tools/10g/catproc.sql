rem 
rem $Header: catproc.sql 06-jul-2006.14:26:10 mxu Exp $ 
rem 
...
Rem     mxu        07/06/06 - Backport mxu_xe_htmldb_auth from st_rdbms_10.2xe 
Rem     bpwang     09/30/05 - move dbmsslrt above catqueue and add prvthlrt
Rem     akoeller   06/28/06 - move catplan before prvtsms 
...
Rem Server-generated alert header files were moved here because of propagation 
Rem calling dbms_server_alert_prvt.post_alert in prvtaqip.plb.  
Rem prvtslrt.sql was split into a header and body file, the body is still loaded
Rem in catalrt.sql.  catalrt.sql and prvthlrt.plb were left in catsvrm.sql
Rem because they depend on the queue APIs defined in catqueue.sql.
Rem 
Rem Also, the creation of alert_type and threshold_type were moved from 
Rem catalrt.sql to dbmsslrt.sql so they would be available at this point.
Rem
Rem Finally, a grant to the datapump role was moved from dbmsslrt to 
Rem catdph.sql below where the role is created.
Rem 
@@dbmsslrt
@@prvthlrt.plb
