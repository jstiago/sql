Rem
Rem $Header: utlu102x.sql 30-jun-2006.03:54:20 rburns Exp $
Rem
Rem utlu102x.sql
Rem
...
DECLARE
  ...
  utlu_version CONSTANT VARCHAR2(30) := '10.2.0.3';
...
-- *****************************************************************
   ...
   apx_missing       BOOLEAN := FALSE; -- TRUE when apxxemig script missing
...
BEGIN
   ...
   IF display_xml THEN
      ...
      IF apx_missing THEN
          DBMS_OUTPUT.PUT_LINE('<warning name="APEX_MISSING"/>');
      END IF;
   ...
   ELSE
      IF version_mismatch or cluster_dbs OR dip_user_exists OR 
         ...
         invalid_objs OR cdc_data or post_upgrade or apx_missing THEN
            display_banner;
      ...
      IF apx_missing THEN
         DBMS_OUTPUT.PUT_LINE(
         'WARNING: --> Application Express requires upgrading but the apex');
         DBMS_OUTPUT.PUT_LINE(
         '.... directory is missing from ORACLE_HOME. Download APEX version 2.2');
         DBMS_OUTPUT.PUT_LINE(
         '.... from OTN before upgrading the XE database.');
      END IF;
...
FUNCTION script_exists(path VARCHAR2, file VARCHAR2) 
RETURN BOOLEAN
IS
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'CREATE OR REPLACE DIRECTORY component_dir AS '''
     || path || '''';
  EXECUTE IMMEDIATE 'GRANT READ ON DIRECTORY component_dir TO DBA';
  EXECUTE IMMEDIATE '
       DECLARE
          file_exists BOOLEAN;
       BEGIN 
          UTL_FILE.FGETATTR(''COMPONENT_DIR'', :file, file_exists, :length, :blocksize);
       IF file_exists THEN
          :is_there := 1;
       END IF;
       END;'
       USING IN file, OUT length, OUT blocksize, OUT is_there;
  EXECUTE IMMEDIATE 'DROP DIRECTORY component_dir';
  IF is_there = 1 THEN
     RETURN TRUE;
  ELSE
     RETURN FALSE;
...
-- *****************************************************************
   ...
   BEGIN
      ...
      IF p_edition = 'XE' THEN 
         ...
         EXECUTE IMMEDIATE 
              'BEGIN dbms_system.get_env(''ORACLE_HOME'',:p_OH_dir); END;'
              USING IN OUT p_OH_dir;
         IF p_OH_dir IS NULL THEN
            NULL;  -- can't get an ORACLE_HOME value to test with
         ELSE
            -- Make sure ORACLE_HOME/rdbms/admin is there
            p_dir := p_OH_dir || '/rdbms/admin';
            IF NOT script_exists (p_dir, 'prvtcr.plb') THEN
               NULL;  -- OH must not be right, so can't find script
            ELSE -- ORACLE_HOME OK, so look for script
               p_dir := p_OH_dir || '/apex/'; 
               IF  NOT script_exists (p_dir, 'apxxemig.sql') THEN
                  apx_missing := TRUE;
               END IF;
            END IF;  -- OH OK
         END IF; -- NO OH
...
-- kbytes for component installs (into SYSTEM and DEFAULT tablespaces)
   ...
   IF dbv = 817 THEN
      ...
      cmp_info(stats).sysaux_kbytes:= 1216+2304;
   ...
   ELSIF dbv = 101 THEN
      ...
      cmp_info(javavm).sys_kbytes:=    9920;  
   ...
   ELSIF dbv = 102 THEN
      ...
      cmp_info(odm).sys_kbytes:=           512;  
      cmp_info(xoq).sys_kbytes:=          1024;  
      cmp_info(sdo).sys_kbytes:=          2048;  
      cmp_info(wk).sys_kbytes:=           2048;  
      cmp_info(em).sys_kbytes:=           2048;  

      cmp_info(catproc).sysaux_kbytes:=   2048;  
      cmp_info(xoq).sysaux_kbytes:=       1536;  

      cmp_info(context).def_ts_kbytes:=        512;  
...
/* avoid CPU dependency in DIAG mode (cpu * 2097152) +  */
   ...
   IF mp(tg_idx).oldvalue != 0 THEN  -- SGA_TARGET in use
      ...
      ELSE
         mp(tg_idx).newvalue := mp(tg_idx).oldvalue;
      END IF;
...
-- *****************************************************************
   ...
   IF dbv !=102 THEN  -- ignore invalid objects on patch upgrades
      BEGIN
         SELECT NULL INTO p_null FROM dba_objects
         WHERE status = 'INVALID' AND 
               object_name NOT LIKE 'BIN$%' AND rownum <=1;
         invalid_objs := TRUE;
      EXCEPTION
         WHEN NO_DATA_FOUND THEN NULL;
      END;
