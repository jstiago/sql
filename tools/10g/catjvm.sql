Rem
Rem $Header: catjvm.sql 12-may-2005.16:36:04 rhlee Exp $
Rem
Rem catjvm.sql
Rem
...
select /*+ ordered use_nl(o m)*/ 
       ...
       decode(m.dbg, 1, 'YES',
                     0, 'NO'),    
       m.src, m.spl, m.oln
from sys.javasnm$ j, sys.obj$ o, sys.x$joxfm m
where o.obj# = m.obn 
  and o.type# = 29
...
select /*+ ordered use_nl(o m)*/ 
       ...
       decode(m.dbg, 1, 'YES',
                     0, 'NO'),    
       m.src, m.spl, m.oln
from sys.javasnm$ j, sys.obj$ o, sys.x$joxfm m, sys.user$ u
where o.obj# = m.obn 
  and o.type# = 29
  and o.owner# = u.user#
  and j.short(+) = o.name
...
(NAME, METHOD_INDEX, METHOD_NAME, ACCESSIBILITY,
       ...
       ARGUMENTS, THROWS,  
       ARRAY_DEPTH, BASE_TYPE, RETURN_CLASS) 
as 
select /*+ ordered use_nl(o m) */
...
as 
select /*+ ordered use_nl(o m) */ nvl(j.longdbcs, o.name), m.mix, m.mnm, m.aix, 
       m.aad, 
       decode(m.abt, 10, 'int',
                     11, 'long',
                     6, 'float',
...
(OWNER, NAME, METHOD_INDEX, METHOD_NAME, ARGUMENT_POSITION, 
       ARRAY_DEPTH, BASE_TYPE, ARGUMENT_CLASS) 
as 
select /*+ ordered use_nl(o m) */ u.name, nvl(j.longdbcs, o.name), m.mix, m.mnm, m.aix, 
       m.aad, 
       decode(m.abt, 10, 'int',
                     11, 'long',
                     6, 'float',
...
as 
select /*+ ordered use_nl(o m) */ nvl(j.longdbcs, o.name), m.mix, m.mnm, m.xix, m.xln 
from sys.javasnm$ j, sys.obj$ o, sys.x$joxfm m
where o.obj# = m.obn 
  and o.type# = 29
  and o.owner# = userenv('SCHEMAID')
...
as 
select /*+ ordered use_nl(o m) */ u.name, nvl(j.longdbcs, o.name), m.mix, m.mnm, m.xix, m.xln 
from sys.javasnm$ j, sys.obj$ o, sys.x$joxfm m, sys.user$ u
where o.obj# = m.obn 
  and o.type# = 29
  and o.owner# = u.user#
...
(SOURCE_NAME, CLASS_INDEX, CLASS_NAME, RESOURCE_INDEX, RESOURCE_NAME)
as 
...
/
comment on column USER_JAVA_DERIVATIONS.RESOURCE_INDEX is 
'index of the derived java resource object'
/ 
comment on column USER_JAVA_DERIVATIONS.RESOURCE_NAME is 
...
(OWNER, SOURCE_NAME, CLASS_INDEX, CLASS_NAME, RESOURCE_INDEX, RESOURCE_NAME)
as 
select u.name,
...
'name of the derived java class object'
/
comment on column ALL_JAVA_DERIVATIONS.RESOURCE_INDEX is 
'index of the derived java resource object'
/ 
comment on column ALL_JAVA_DERIVATIONS.RESOURCE_NAME is 
'name of the derived java resource object'
/
...
'name of the derived java class object'
/
comment on column DBA_JAVA_DERIVATIONS.RESOURCE_INDEX is 
'index of the derived java resource object'
/ 
comment on column DBA_JAVA_DERIVATIONS.RESOURCE_NAME is 
'name of the derived java resource object'
/
...
as 
select /*+ ordered use_nl(o m) */ nvl(j.longdbcs, o.name),
       t.joxftncompsource,
       t.joxftncompinitializer,
       t.joxftncomplibraryfile,
       t.joxftncomplibrary
from sys.javasnm$ j, sys.obj$ o, sys.x$joxft t
where o.obj# = t.joxftobn
  and o.type# = 29
  and o.owner# = userenv('SCHEMAID')
  and j.short(+) = o.name
...
select /*+ ordered use_nl(o m) */
       ...
       t.joxftncomplibraryfile,
       t.joxftncomplibrary
from sys.javasnm$ j, sys.obj$ o, sys.x$joxft t, sys.user$ u
where o.obj# = t.joxftobn
  and o.type# = 29
  and o.owner# = u.user#
  and j.short(+) = o.name
