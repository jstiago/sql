Rem
Rem $Header: i0801070.sql 14-nov-2005.09:57:39 rburns Exp $
Rem
Rem i0801070.sql
Rem
...
Rem    rburns      11/14/05 - Backport rburns_bug-4685014 from st_rdbms_10.2 
...
Rem which is 'SYSTEM' - do not insert anything if it already exists

insert into props$ (name, value$, comment$)
    (select 'DEFAULT_TEMP_TABLESPACE', 'SYSTEM',
            'ID of default temporary tablespace' from dual
    where 'DEFAULT_TEMP_TABLESPACE' NOT IN
    (select name from props$ where name =  'DEFAULT_TEMP_TABLESPACE'));
commit;
