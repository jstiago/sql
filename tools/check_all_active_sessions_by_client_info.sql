...
BEGIN
  ...
  FOR c_SESSION IN (SELECT    s.INST_ID
                             ...
                             ,s.P3RAW
                    FROM      GV$SESSION s
                    WHERE     s.CLIENT_INFO = v_CLIENT_INFO     
                    AND       s.STATUS    = 'ACTIVE'
  ...
  LOOP
    ...
    DBMS_OUTPUT.PUT_LINE(  'PGA Memory      : ' || TO_CHAR(v_PGA_USED_MEM) || ' used; ' || TO_CHAR(v_PGA_ALLOC_MEM) || ' alloc; ' || TO_CHAR(v_PGA_FREEABLE_MEM) || ' freeable');
